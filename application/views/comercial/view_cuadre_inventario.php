<?php
	if ($this->input->post('cantidad')){
		$cantidad = array('name'=>'cantidad','id'=>'cantidad','maxlength'=>'10','value'=>$this->input->post('cantidad'), 'style'=>'width:70px;visibility: visible;height: 24px;', 'class'=>'required', 'onpaste'=>'return false');
	}else{
		$cantidad = array('name'=>'cantidad','id'=>'cantidad','maxlength'=>'10', 'style'=>'width:70px;visibility: visible;height: 24px;', 'class'=>'required', 'onpaste'=>'return false');
	}

	if ($this->input->post('nombre_producto')){
	    $nombre_producto = array('name'=>'nombre_producto','id'=>'nombre_producto','value'=>$this->input->post('nombre_producto'), 'style'=>'width:265px;font-family: verdana;visibility: visible;height: 24px;','placeholder'=>' :: Nombre del Producto ::');
	}else{
	    $nombre_producto = array('name'=>'nombre_producto','id'=>'nombre_producto', 'style'=>'width:265px;font-family: verdana;visibility: visible;height: 24px;','placeholder'=>' :: Nombre del Producto ::'); 
	}

	if ($this->input->post('stockactual')){
	    $stockactual = array('name'=>'stockactual','id'=>'stockactual','maxlength'=> '10', 'value' => $this->input->post('stockactual'), 'style'=>'width:100px;visibility: visible;height: 24px;','readonly'=> 'readonly');
	}else{
	    $stockactual = array('name'=>'stockactual','id'=>'stockactual','maxlength'=> '10', 'style'=>'width:100px;visibility: visible;height: 24px;','readonly'=> 'readonly');
	}
	...
	if ($this->input->post('nombre_producto_o_i')){
	    $nombre_producto_o_i = array('name'=>'nombre_producto_o_i','id'=>'nombre_producto_o_i','value'=>$this->input->post('nombre_producto_o_i'), 'style'=>'width:265px;font-family: verdana;visibility: visible;height: 24px;','placeholder'=>' :: Nombre del Producto ::');
	}else{
	    $nombre_producto_o_i = array('name'=>'nombre_producto_o_i','id'=>'nombre_producto_o_i', 'style'=>'width:265px;font-family: verdana;visibility: visible;height: 24px;','placeholder'=>' :: Nombre del Producto ::'); 
	}

	if ($this->input->post('cantidad_o_i')){
		$cantidad_o_i = array('name'=>'cantidad_o_i','id'=>'cantidad_o_i','maxlength'=>'10','value'=>$this->input->post('cantidad_o_i'), 'style'=>'width:70px;visibility: visible;height: 24px;', 'class'=>'required', 'onpaste'=>'return false');
	}else{
		$cantidad_o_i = array('name'=>'cantidad_o_i','id'=>'cantidad_o_i','maxlength'=>'10', 'style'=>'width:70px;visibility: visible;height: 24px;', 'class'=>'required', 'onpaste'=>'return false');
	}

	if ($this->input->post('stockactual_o_i')){
	    $stockactual_o_i = array('name'=>'stockactual_o_i','id'=>'stockactual_o_i','maxlength'=> '10', 'value' => $this->input->post('stockactual_o_i'), 'style'=>'width:100px;visibility: visible;height: 24px;','readonly'=> 'readonly');
	}else{
	    $stockactual_o_i = array('name'=>'stockactual_o_i','id'=>'stockactual_o_i','maxlength'=> '10', 'style'=>'width:100px;visibility: visible;height: 24px;','readonly'=> 'readonly');
	}
?>
		...
		if(nombre_producto == '' || cantidad == '' || area == ''){
	    	...
	    	$.ajax({
	            ...
	            url: "<?php echo base_url(); ?>comercial/cuadrar_producto_area_almacen/",
	          	...
	          	success: function(response){
		            if(response == 1){
		            	swal({ title: "El Cuadre del Producto en Almacén se realizó con Éxito!",text: "",type: "success",confirmButtonText: "OK",timer: 2000 });
		              	$('#area').val('');
		              	$('#nombre_producto').val('');
...
<div id="contenedor">
	...
	<div id="formFiltro">
		<!--Contenedor-->
		<div id="datos_factura_importada" style="background: whitesmoke;width: 1370px;border-bottom: 1px solid #000;padding-left: 10px;margin-bottom: 15px;height: 220px;">
			<div id="container_tabs" style="margin-bottom: 10px;">
				<!--Pestaña 1 activa por defecto-->
			    <input id="tab-1" type="radio" name="tab-group" checked="checked" style="display: none;" />
			        ...
			        <div id="content-1">
						<div id="data_general" style="opacity: inherit;padding: 0.1em;width: 400px;float: left;">
						    ...
						    <table width="614" border="0" cellspacing="0" cellpadding="0" style="margin-top: 3px;">			        
							    <tr>
							       	<td width="117" style="padding-top: 3px;" colspan="6"><input type="submit" id="cuadre_almacen" name="cuadre_almacen" value="CUADRAR INVENTARIO" style="padding-bottom:3px; padding-top:3px; margin-bottom: 4px; background-color: #FF5722; border-radius:6px; margin-left: 300px; width: 150px;visibility: visible;height: 22px;" /></td>
							    </tr>						        
							</table>
						</div>
					</div>
