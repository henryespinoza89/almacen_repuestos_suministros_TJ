...
<script type="text/javascript">
  $(function(){
    ...
    $('#listaCategoriaProductos').jTPS( {perPages:[10,15,20,'Todos'],scrollStep:1,scrollDelay:30,clickCallback:function () {     
            // target table selector
            var table = '#listaCategoriaProductos';
            ...
            if (cookie[0] == 'jTPS') {
                    ...
                    for (var cm = 0, cme = commands.length; cm < cme; cm++) {
                            ...
                            if (command[0] == 'sortasc' && parseInt(command[1]) >= 0) {
                                    $('#listaCategoriaProductos .sortableHeader:eq(' + parseInt(command[1]) + ')').click();
                            } else if (command[0] == 'sortdesc' && parseInt(command[1]) >= 0) {
                                    $('#listaCategoriaProductos .sortableHeader:eq(' + parseInt(command[1]) + ')').click().click();
                            } else if (command[0] == 'page' && parseInt(command[1]) >= 0) {
                                    $('#listaCategoriaProductos .pageSelector:eq(' + parseInt(command[1]) + ')').click();
                            }
                    }
            }
    }
    // bind mouseover for each tbody row and change cell (td) hover style
    $('#listaCategoriaProductos tbody tr:not(.stubCell)').bind('mouseover mouseout',
            function (e) {
                    // hilight the row
        ...
        buttons: {
          'Eliminar': function () {
            var parent = $(this).data('parent');
                ...
                $(this).dialog('close');
                setTimeout('window.location.href="<?php echo base_url(); ?>comercial/gestioncategoriaproductos"', 200);
        },
            'Cancelar': function () {
                  $(this).dialog('close');
            }
...
<body>
  <div id="contenedor">
    <div id="tituloCont">Registrar Categoría de Producto</div>
    <div id="formFiltro">
        ...
        <?php 
          ...
          {
        ?>
        <table border="0" cellspacing="0" cellpadding="0" id="listaCategoriaProductos">
          <thead>
            <tr class="tituloTable">
              <td sort="idproducto" width="80" height="25">Item</td>
              <td sort="nombreprod" width="200">Categoría de Producto</td>
              <td width="20">&nbsp;</td>
              <td width="20">&nbsp;</td>
            </tr>
          </thead>
          ...
          <tr class="contentTable">
            <!--<td><?php //echo str_pad($data->id_nombre_maquina, 5, 0, STR_PAD_LEFT); ?></td>-->
            <td height="27"><?php echo str_pad($i,4,0, STR_PAD_LEFT);?></td>
            <td><?php echo $data->no_categoria; ?></td>
            <td width="20" align="center"><img class="editar_categoria_producto" src="<?php echo base_url();?>assets/img/edit.png" width="20" height="20" title="Editar Categoría de Producto" onClick="editar_categoria_producto(<?php echo $data->id_categoria; ?>)" /></td>
